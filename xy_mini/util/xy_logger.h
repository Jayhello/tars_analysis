...
#include "xy_ec.h"
#include "xy_file.h"
#include "xy_thread_queue.h"
#include "xy_cas_queue.h"
#include "xy_timeprovider.h"
#include "xy_spin_lock.h"
...
public:
     ...
     * @param mutex
     */
    LoggerStream(const char *header, ostream *stream, ostream *estream, TC_SpinLock &mutex) : _stream(stream),
                                                                                              _estream(estream),
                                                                                              _mutex(mutex) {
        if (stream) {
            _buffer << header;
...
public:
    ...
    */
     ...
     */
    TC_Logger()
            : _flag(HAS_TIME), _level(DEBUG_LOG_LEVEL), _buffer(TC_LoggerRollPtr::dynamicCast(this->_roll), 1024),
              _stream(&_buffer), _ebuffer(NULL, 0), _estream(&_ebuffer), _sSepar("|"), _bHasSquareBracket(false) {
    }
...
protected:
    ...
    void head(char *c, int len, int level) {
        ...
        if (hasFlag(TC_Logger::HAS_MTIME)) {
            ...
            const char *szFormat = (_bHasSquareBracket) ? ("[%04d-%02d-%02d %02d:%02d:%02d.%03ld]%s")
            ...
            // 				localtime_s(&tt, &t);
            // #endif
            const char *szFormat = (_bHasSquareBracket) ? ("[%04d-%02d-%02d %02d:%02d:%02d]%s")
...
public:
    ...
    void
    init(const string &path, const string &format = "%Y%m%d", bool bHasSufix = true, const string &sConcatstr = "_",
