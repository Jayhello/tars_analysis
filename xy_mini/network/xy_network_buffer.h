...
public:
    ...
    */
    ...
    public:
        ...
        char &at(size_t offset) {
            if (_pos + offset >= _buffer.size())
                throw TC_NetWorkBuffer_Exception(
                        "[TC_NetWorkBuffer::Buffer] at '" + TC_Common::tostr(offset) + "' offset overflow");
     ...
     * @param buff
     */
    void setContextData(void *contextData, std::function<void(TC_NetWorkBuffer *)> deconstruct = std::function<void(
            TC_NetWorkBuffer *)>()) {
    ...
    T getHeader(size_t len) const {
        if (getBufferLength() < len) {
            throw TC_NetWorkBuffer_Exception(
                    "[TC_NetWorkBuffer::getHeader] no enough buff(" + TC_Common::tostr(getBufferLength()) +
                    ") to get(" + TC_Common::tostr(len) + ")");
        }
