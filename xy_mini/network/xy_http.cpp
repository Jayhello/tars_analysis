...
void TC_Http::setHeader(const string &sHeadName, const string &sHeadValue) {
    ...
    if ((TC_Port::strcasecmp(sHeadName.c_str(), pStr1) != 0) &&
        (TC_Port::strcasecmp(sHeadName.c_str(), pStr2) != 0)) {
...
bool TC_HttpCookie::matchDomain(const string &sCookieDomain, const string &sDomain) {
    ...
    if (sDomain.length() >= sCookieDomainNew.length() &&
        sLowerDomain.compare(sDomain.length() - sCookieDomainNew.length(), sCookieDomainNew.length(),
                             sCookieDomainNew) == 0) {
        return true;
    }
...
void
TC_HttpRequest::setRequest(const string &method, const string &sUrl, const std::string &body, bool bNewCreateHost) {
...
void TC_HttpRequest::parseRequestHeader(const char *szBuffer, const char *header) {
    ...
    if (f1 == p) {
        throw TC_HttpRequest_Exception("[TC_HttpRequest::parseRequestHeader] http request format error: " +
...
const string &TC_HttpRequest::getMethod() const {
    ...
    if (it != TC_Http::HEADER_REVERSE.end()) {
        ...
        throw TC_HttpRequest_Exception(
                "[TC_HttpRequest::setMethod] http request command error: " + TC_Common::tostr(_requestType));
    }
}
...
void TC_HttpRequest::setMethod(const char *sMethod) {
    ...
    if (it != TC_Http::HEADER.end()) {
        ...
        throw TC_HttpRequest_Exception(
                "[TC_HttpRequest::setMethod] http request command error: " + string(sMethod));
    }
}
